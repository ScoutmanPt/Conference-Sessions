...
Describe 'New-CCConnection' {
    ...
    BeforeEach {
        ...
        "TenantId=tenant-id-1`nClientId=app-id-1" | Set-Content $script:configPath
    ...
    Context 'parameter validation' {
        ...
        It 'throws when ConfigPath does not exist' {
            { New-CCConnection @script:baseParams -ConfigPath 'C:\no\config.ini' } |
                Should -Throw '*config.ini not found*'
        }
    }

    Context 'credential validation' {
        It 'throws when secret ClientId does not match config ClientId' {
        ...
        It 'wraps a SecureString secret using config ClientId' {
            Mock Get-Secret { ConvertTo-SecureString 'secret' -AsPlainText -Force }
            # Should not throw — the SecureString branch wraps it with config.ClientId
