...
.DESCRIPTION
    ...
    vault, connects to Microsoft Graph, then recreates the external connection
...
#>
    ...
    if ($credential.UserName -ne $config.ClientId) {
        throw "Secret '$SecretName' contains client id '$($credential.UserName)', but config.ini contains '$($config.ClientId)'. Re-run Register-CCApp so the secret and config are recreated together."
    }
    ...
    while (-not $mgReady) {
        try {
            ...
            Connect-MgGraph -ClientSecretCredential $credential -TenantId $config.TenantId -NoWelcome -ContextScope Process -ErrorAction Stop
